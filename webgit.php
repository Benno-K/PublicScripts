...
if ($level == 1) {
    ...
    $cmd = sprintf('%s -C %s show --color %s 2>&1',
...
<head>
    ...
    #headline-row {
        ...
        background: var(--headline-bg, #0d1117);
    ...
    .levelup-btn {
        display: inline-block; background: var(--btn-bg, #388bfd); color: #fff;
    ...
    .levelup-btn:hover { background: var(--btn-bg-hover, #58a6ff); transform: scale(1.05);}
    .theme-switcher {
        display: inline-block; background: var(--btn-bg, #388bfd); color: #fff;
        border: none; padding: 0.42em 1.3em; border-radius: 7px; font-size: 1.09em;
    ...
    .theme-switcher:hover { background: var(--btn-bg-hover, #58a6ff); transform: scale(1.05);}
    .theme-popup {
        display: none; position: absolute; top: 110%; right: 0; min-width: 9em;
        background: var(--popup-bg, #161b22); border: 1px solid var(--border-color, #21262d);
    ...
    .theme-popup .theme-item {
        display: block; color: #fff; padding: 0.45em 1.2em; border: none;
        background: none; width: 100%; text-align: left; font-size: 1.06em;
        cursor: pointer; transition: background 0.11s;
        border-radius: 3px;
    }
    .theme-popup .theme-item.selected { font-weight: 600; color: #58a6ff;}
    .theme-popup .theme-item:hover { background: var(--btn-bg-hover, #222b); color: #fff;}
    @media print {
        .theme-switcher, .theme-popup, .levelup-btn { display: none !important;}
        #headline-row { border-bottom: none !important;}
    }
    .subheadline { text-align: center; font-size: 1.18em; margin: 2em 0 1.2em 0; color: var(--subheadline-color, #58a6ff);}
    .nav-content-layout {
        display: flex; flex-direction: row; align-items: stretch; min-height: 80vh;
    }
    .nav-pane {
        flex: 0 0 18em; max-width: 25vw; min-width: 12em; background: var(--nav-bg, #161b22);
        border-right: 1px solid var(--border-color, #21262d); padding: 1.5em 1em 1em 1.3em;
    }
    .main-pane {
...
elseif ($level == 2 && !$notfound): ?>
    <div class="nav-content-layout">
        <div class="nav-pane">
            <div style="font-size:1.2em; font-weight:600; color:var(--subheadline-color,#58a6ff); margin-bottom:1em;">
                <?=htmlspecialchars($repo)?>
            </div>
        ...
        <div class="main-pane">
            ...
            <?php else: ?>
                <div>
                <?php foreach ($commits as $c): ?>
                    <div style="padding:0.31em 0; border-bottom:1px solid var(--border-color,#21262d); display:flex; align-items:center; gap:0.8em;">
                        ...
                        <span style="color:#8b949e; min-width:8ch;"><?=htmlspecialchars($c['date'])?></span>
                        <span style="color:#39d353; min-width:8ch;"><?=htmlspecialchars($c['author'])?></span>
                        <span style="color:#c9d1d9; margin-left:0.5em; flex:1;"><?=htmlspecialchars($c['subject'])?></span>
                    </div>
                <?php endforeach; ?>
                </div>
            <?php endif; ?>
...
elseif ($level == 3 && !$notfound): ?>
    <div class="nav-content-layout">
        <div class="nav-pane">
            ...
            <?php foreach ($commits as $c): ?>
                <li style="margin-bottom:0.39em;">
                    <a href="gitweb.php?repo=<?=urlencode($repo)?>&commit=<?=$c['hash']?>" class="levelup-btn" style="font-family:monospace; font-size:1em;<?=($c['hash']==$commit?' background:var(--btn-bg-hover,#58a6ff);':'')?>">
                        <?=htmlspecialchars($c['hash'])?>
                    </a>
                </li>
            <?php endforeach; ?>
        ...
        <div class="main-pane">
            <div style="margin-bottom:2em;">
                <span style="font-size:1.16em; color:var(--subheadline-color,#58a6ff); font-weight:600;"><?=htmlspecialchars($repo)?> / <span style="font-family:monospace;"><?=htmlspecialchars($commit)?></span></span>
            </div>
